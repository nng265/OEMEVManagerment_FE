    ...
    <DetailSection title="Details for Technician">
      <div className="detail-grid" style={{ gridTemplateColumns: "1fr" }}>
        <div className="detail-item">
          <span className="value" style={{ whiteSpace: "pre-wrap", color: "#2d3748" }}>
            {claimData.notes}
          </span>
        </div>
      </div>
  ...
  const assignTechnicianSection = (
    <DetailSection title="Assign Technician">
      <div className="technicians-section">
            ...
            {loading ? (
              ...
              >
                ...
                {availableTechs.map(techOpt => {
                  ...
                  const isAvailable = !selectedIds.includes(techOpt.userId) || techOpt.userId === tech.selectedValue;
        ...
        {error && !loading && (
          <div className="select-error" style={{ marginTop: '10px', color: 'red' }}>{error}</div>
        )}
      </div>
    </DetailSection>
  );
  ...
  return (
    <form onSubmit={handleSubmit}>
      <WarrantyClaimDetailModal
        ...
        title={`Assign Technician - ${claimData.status || 'Waiting for Unassigned'}`}
        showBackButton={true}
