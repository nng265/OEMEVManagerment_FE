...
export const WorkOrderDetailModal = ({
  ...
  const handleSubmitInspection = async () => {
    if (
      ...
      alert("Vui lòng nhập mô tả hoặc tải lên ít nhất một hình ảnh!");
      return;
    }
    ...
    try {
      ...
      if (typeof submitInspection === "function") {
        const res = await submitInspection(warrantyInfo?.claimId, {
          body: payload,
        });
        console.log(" Inspection submitted:", res);
        alert("Đã lưu kết quả kiểm tra thành công!");
      } else {
        console.warn(" submitInspection chưa được truyền từ container");
      }
      onClose();
    } catch (err) {
  ...
  const handleSubmitRepair = async () => {
    const invalid = parts.some(
      (p) => !p.action || !p.category || !p.model || !p.serial || !p.newSerial
    );
    if (invalid) {
    ...
    try {
      ...
      const payload = { replacedParts: parts };

      if (typeof submitRepair === "function") {
        const res = await submitRepair(warrantyInfo?.claimId, {
          body: payload,
        });
