...
export function AccessoryListContainer() {
  ...
  useEffect(() => {
    ...
    const mapToOptions = (arr, emptyLabel) => {
    ...
    const fetchOptions = async () => {
      try {
        ...
        setOrgOptions(mapToOptions(orgData, "All Organizations"));
  ...
  const fetchAccessory = useCallback(
    async (page = 0, size, searchText, statusText, conditionText, orgText) => {//mới
      ...
      try {
        ...
        if (effectiveOrg?.trim()) params.Organization = effectiveOrg; // pass organization name
